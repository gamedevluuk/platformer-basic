...
public class UseLineRenderer : MonoBehaviour
{
    ...
    private int _balloonColliderCount = 4;
    private List<GameObject> _balloonColliders;

    private void UpdateRopeCollider()
    {
        //We create the Collider used for the rope here if it doesnt exist yet.
        //If it does we simply update its position.
        if (_balloonColliders == null)
        {
            _balloonColliders = new List<GameObject>();

            for (int i = 0; i < _balloonColliderCount; i++)
            {
                continue;
                var newCollider = new GameObject("Collider");
                newCollider.AddComponent<CapsuleCollider2D>();
                newCollider.AddComponent<Rigidbody2D>();

                newCollider.GetComponent<Rigidbody2D>().mass = 0.0001f;
            }
        }
    }
